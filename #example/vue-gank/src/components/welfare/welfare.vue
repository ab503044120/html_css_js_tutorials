<template>
  <div>
    <div v-show="!showDetail" class="welfare-wrapper" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10">
      <div class="welfare-center">
        <figure v-show="leftData.length > 0" v-for="(data,index) in leftData" @click="selectDetails(data.createdAt)" :key='index'>
          <v-img :imgUrl="data.url"></v-img>
        </figure>
      </div>
      <div class="welfare-center">
        <figure v-show="rightData.length > 0" v-for="(data,index) in rightData" @click="selectDetails(data.createdAt)" :key='index'>
          <v-img :imgUrl="data.url"></v-img>
        </figure>
      </div>
    </div>
    <v-details ref="details" key="detail" :isShow="showDetail" :time="time" :detailsData="detailsData" @hideDetail="hideDetail"></v-details>
  </div>
</template>
